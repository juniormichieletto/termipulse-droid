...
object AnsiParser {
    ...
    private fun AnnotatedString.Builder.appendSegment(
        ...
        var lastIndex = 0
        val urlMatches = URL_REGEX.findAll(text)

        for (match in urlMatches) {
            val rawStart = match.range.first
            val rawEnd = match.range.last + 1
            val rawUrl = text.substring(rawStart, rawEnd)

            val (cleanUrl, trailingPunctuation) = cleanUrlAndPunctuation(rawUrl)

            if (rawStart > lastIndex) {
                val prefix = text.substring(lastIndex, rawStart)
                appendStyledText(prefix, color, isBold, isItalic, isUnderline)
            }

            if (cleanUrl.isNotEmpty()) {
                val urlStartInBuilder = length
                val linkColor = Color(0xFF80D8FF)
                ...
                append(cleanUrl)
                pop()
                val urlEndInBuilder = length

                addStringAnnotation(
                    tag = "URL",
                    annotation = cleanUrl,
                    start = urlStartInBuilder,
                    end = urlEndInBuilder
                )
            }

            if (trailingPunctuation.isNotEmpty()) {
                appendStyledText(trailingPunctuation, color, isBold, isItalic, isUnderline)
            }
        ...
        if (lastIndex < text.length) {
            ...
            appendStyledText(remaining, color, isBold, isItalic, isUnderline)
        }
    }

    private fun cleanUrlAndPunctuation(rawUrl: String): Pair<String, String> {
        var url = rawUrl
        var trailing = ""
